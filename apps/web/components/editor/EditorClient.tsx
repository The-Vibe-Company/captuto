...
export function EditorClient({
  ...
  const handleTitleChange = useCallback(async (newTitle: string) => {
    ...
    try {
      ...
      // The persisted title was restored; step-save status remains independent.
  ...
  const handleCreateStepFromSource = useCallback(async (source: SourceWithSignedUrl) => {
    const response = await fetch('/api/steps', {
      ...
      body: JSON.stringify({
        ...
        source_id: source.id,
        step_type: 'image',
        text_content: source.auto_caption || undefined,
      }),
    });
    if (!response.ok) throw new Error('The screen could not be added. Please try again.');
  ...
  const handleApplyGenerated = useCallback(async (generated: GeneratedTutorialContent) => {
    ...
    try {
      const metadataResponse = await fetch(`/api/tutorials/${tutorial.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title: generated.title || tutorial.title, description: generated.description }),
      });
      if (!metadataResponse.ok) throw new Error('The guide title and description could not be saved. Your generated draft is still available.');
      setTutorial(current => ({ ...current, title: generated.title || current.title, description: generated.description }));
      ...
      if (stepUpdates.length > 0) {
        ...
        if (!response.ok) throw new Error('Generated steps could not be saved. Your draft is still available; retry to continue.');
        if (response.ok) {
          setSteps((prev) =>
            prev.map((step) => {
      ...
      for (const toCreate of sourcesToCreate) {
        ...
        if (!source) throw new Error('A generated screen is no longer available. Please generate again.');
        if (source) {
          // Create step via API
          const response = await fetch('/api/steps', {
            method: 'POST',
